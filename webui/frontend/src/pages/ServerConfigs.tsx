...
export default function ServerConfigs({
  serverId,
  onClose,
}: ServerConfigsProps) {
  ...
  useEffect(() => {
    axios
      .get(`/api/servers/${serverId}/configs`)
      .then((res) =>
        setConfigs(res.data.configs.map((p: string) => p.split(/[/\\]/).pop())),
      )
  ...
  const loadConfig = (cfg: string) => {
    ...
    setError("");
    axios
      .get(`/api/servers/${serverId}/configs/${cfg}`)
      .then((res) => {
        setSelected(cfg);
  ...
  const saveConfig = () => {
    ...
    setSaveMsg("");
    axios
      .put(`/api/servers/${serverId}/configs/${selected}`, { content })
      .then(() => {
        setOriginal(content);
        setSaveMsg("Saved!");
      })
