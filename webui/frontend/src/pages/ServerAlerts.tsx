...
export default function ServerAlerts({ serverId, onClose }: ServerAlertsProps) {
  ...
  useEffect(() => {
    axios.get(`/api/servers/${serverId}/alerts`)
  ...
  const loadAlert = (alert: string) => {
    setLoading(true);
    setError('');
    axios.get(`/api/servers/${serverId}/alerts/${alert}`)
      .then(res => {
        setSelected(alert);
  ...
  const saveAlert = () => {
    setSaving(true);
    setSaveMsg('');
    axios.put(`/api/servers/${serverId}/alerts/${selected}`, { content })
      .then(() => {
        setOriginal(content);
        setSaveMsg('Saved!');
      })
  ...
  const testAlert = () => {
    setTestMsg('');
    axios.post(`/api/servers/${serverId}/alerts/${selected}/test`)
            ...
            {alerts.map(a => (
              <li key={a.script}>
                <button
                  ...
                  disabled={loading}
                >
                  {a.script.replace('alert_', '').replace('.sh', '')} {a.enabled ? '' : '(disabled)'}
                </button>
              </li>
            ))}
          </ul>
        ...
        <div className="flex-1">
          {selected && (
            <div>
              ...
              {testMsg && <pre className="bg-black text-green-300 p-2 rounded text-xs overflow-x-auto max-h-32 whitespace-pre-wrap">{testMsg}</pre>}
            </div>
          )}
        </div>
      </div>
