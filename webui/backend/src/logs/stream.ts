...
export function registerLogStreamWS(app: FastifyInstance) {
  app.get('/api/servers/:id/logs/stream', { websocket: true }, (connection, req) => {
    ...
    // Tail the log file
    const stream = fs.createReadStream(logPath, { encoding: 'utf-8', start: Math.max(0, fs.statSync(logPath).size - 4096) });
    stream.on('data', chunk => connection.socket.send(chunk));
    // Watch for new data
    const watcher = fs.watch(logPath, (event) => {
      if (event === 'change') {
        const newStream = fs.createReadStream(logPath, { encoding: 'utf-8', start: fs.statSync(logPath).size - 1024 });
