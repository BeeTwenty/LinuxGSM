...
export function registerConsoleWS(app: FastifyInstance) {
  app.get('/api/servers/:id/console', { websocket: true }, (connection, req) => {
    ...
    const tmux = spawn('tmux', ['attach-session', '-t', session], { stdio: ['pipe', 'pipe', 'pipe'] });
    tmux.stdout.on('data', data => connection.socket.send(data.toString()));
    tmux.stderr.on('data', data => connection.socket.send('[stderr] ' + data.toString()));
