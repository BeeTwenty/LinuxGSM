...
export function registerServerDetailsAPI(app: FastifyInstance) {
  app.get('/api/servers/:id/details', async (request, reply) => {
    ...
    try {
      const detailsPath = path.join(path.dirname(server.scriptPath), 'log', `${server.name}-details.log`);
      let details = '';
      if (fs.existsSync(detailsPath)) {
        details = fs.readFileSync(detailsPath, 'utf-8');
      }
      return { id: server.id, name: server.name, scriptPath: server.scriptPath, details };
    } catch (err: any) {
