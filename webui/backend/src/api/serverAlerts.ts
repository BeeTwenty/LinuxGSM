...
function getAlertConfigPath(serverScript: string, alertScript: string) {
  return path.join(path.dirname(serverScript), 'lgsm', 'config-lgsm', alertScript.replace('.sh', '.cfg'));
}
...
export function registerServerAlertsAPI(app: FastifyInstance) {
  ...
  app.get('/api/servers/:id/alerts/:alertId', async (request, reply) => {
    ...
    const server = servers.find(s => s.id === id);
    if (!server) return reply.code(404).send({ error: 'Server not found' });
    if (!ALERT_CONFIGS.includes(alertId)) return reply.code(403).send({ error: 'Alert not allowed' });
    const configPath = getAlertConfigPath(server.scriptPath, alertId);
    if (!fs.existsSync(configPath)) return reply.code(404).send({ error: 'Config not found' });
  ...
  app.post('/api/servers/:id/alerts/:alertId/test', async (request, reply) => {
    ...
    const proc = spawn(scriptPath, ['test-alert', alertId.replace('alert_', '').replace('.sh', '')], {
      env: process.env,
