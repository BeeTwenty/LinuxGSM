...
export function registerServerConfigAPI(app: FastifyInstance) {
  ...
  app.get('/api/servers/:id/configs/:configId', async (request, reply) => {
    ...
    const server = servers.find(s => s.id === id);
    if (!server) return reply.code(404).send({ error: 'Server not found' });
    if (!ALLOWED_CONFIGS.includes(configId)) return reply.code(403).send({ error: 'Config not allowed' });
    const configPath = getConfigPaths(server.scriptPath).find(p => p.endsWith(configId));
    if (!configPath || !fs.existsSync(configPath)) return reply.code(404).send({ error: 'Config not found' });
